...
在Python的世界中，一切皆对象。小到一个整数，一个字符串都是对象。甚至连 类型、函数等都是一种对象。

在Python中，有一些已经定义好的类型对象，如 int类型、string类型、dict类型等，
这些对象我们称为内建类型对象，这些类型对象通过“实例化”可以创建内建类型对象的实例对象，
如 int对象、string对象、dict对象。
